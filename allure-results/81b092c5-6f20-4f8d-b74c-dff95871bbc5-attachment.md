...
Call log:
  - waiting for getByRole('link', { name: /view procing/i }).first()
...
```ts
  ...
  36 | 
  37 |         viewPricingButton = await page.getByRole('link', {name: /view procing/i}).first();
> 38 |         await viewPricingButton.scrollIntoViewIfNeeded();
  ...
  44 |         console.log(`Best Dollar Value Amount: ${bestDollarValueAmount}`);
  45 | 
  46 |     });
