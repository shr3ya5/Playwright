...
```
Error: locator.waitFor: Error: strict mode violation: locator('li#fourth') resolved to 2 elements:
...
```ts
  ...
  76 | 
  77 |         let optionName = await page.locator('li#fourth'); 
  78 |         let target = await page.locator('div#shoppingCart1 li.placeholder');
> 79 |         await optionName.waitFor({state:"attached"});
     |                          ^ Error: locator.waitFor: Error: strict mode violation: locator('li#fourth') resolved to 2 elements:
  80 |         optionName.dragTo(target);
  81 |         
  82 |     });
  83 |     test.afterEach('Close Page', async() => {
  84 |         await page.close();
  85 |     });
  86 |     test.afterAll('Close Browser', async() => {
