...
Call log:
  - waiting for locator('#dropdown-menu').locator('#dropdown-menu label[value="Apple"]') to be visible
...
```ts
  ...
  36 | 
  37 |         let AppleOption = await selectFruitsDD.locator('#dropdown-menu label[value="Apple"]');
> 38 |         await AppleOption.waitFor({state:'visible'});
     |                           ^ Error: locator.waitFor: Target page, context or browser has been closed
  39 |         await AppleOption.click({delay: 1000});
  40 | 
  41 |         let BananaOption = await selectFruitsDD.locator('#dropdown-menu label[value="Banana"]');
  42 |         await BananaOption.waitFor({state:'visible'});
