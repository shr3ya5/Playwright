...
Call log:
  - Expect "toHaveText" locator('h6') with timeout 10000ms
...
```ts
  ...
  18 |             await test.step('Navigate to Orange HRM Login Page', async () => {
  ...
  22 |             });
  23 |             await test.step('Perform login with credentials', async () => {
