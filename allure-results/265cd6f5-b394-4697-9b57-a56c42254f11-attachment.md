...
```ts
  ...
  54 | 
  55 |         await colorsList.selectOption(['Red', 'Blue', 'Green']);
> 56 |         await expect(colorsList).toHaveValues(['Red', 'Blue', 'Green']);
     |                                  ^ Error: expect(locator).toHaveValues(expected) failed
  57 |     });
  58 |     test.afterEach('Close Page', async() => {
  59 |         await page.close();
  60 |     });
  61 |     test.afterAll('Close Browser', async() => {
