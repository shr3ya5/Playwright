...
Call log:
  - waiting for getByRole('link', { name: /talk to us/i }).first()
...
```ts
  ...
  48 | 
  49 |         let talkToUsButton = await page.getByRole('link', {name: /talk to us/i }).first();
  50 |         //use control.evaluate(el => getComputedStyle(el).backgroundColor) to fetch the background color
> 51 |         await talkToUsButton.dblclick();
     |                              ^ Error: locator.dblclick: Target page, context or browser has been closed
  52 | 
  53 |         let popupTitle = await page.locator('#popUpCsfModalTitle2');
  54 |         console.log(`Popup Title: ${popupTitle.innerText()}`);
  55 |     });
  56 |     test.afterEach('Close Page', async() => {
  57 |         await page.close();
  58 |     });
  59 |     test.afterAll('Close Browser', async() => {
