...
Call log:
  - waiting for locator('#dropdown-menu').locator('#dropdown-menu label').filter({ hasText: 'Apple' })
...
```ts
  ...
  36 | 
  37 |         let AppleOption = await selectFruitsDD.locator('#dropdown-menu label').filter({hasText: 'Apple'});
> 38 |         await AppleOption.click({delay: 1000});
     |                           ^ Error: locator.click: Target page, context or browser has been closed
  39 | 
  40 |         let BananaOption = await selectFruitsDD.locator('#dropdown-menu label').filter({hasText: 'Banana'});
  41 |         await BananaOption.click({delay: 1000});
  42 | 
