...
```
Error: locator.innerText: Unexpected token " " while parsing css selector "tbody tr: nth-child(2)". Did you mean to CSS.escape it?
Call log:
  - waiting for internal:role=table[name="BookTable"i] >> tbody tr: nth-child(2)
...
```ts
  ...
  51 |         await uploadFile.setInputFiles('./data/Push_Playwright_Code_to_GitHub_Step_by_Step.txt');
  52 |         await uploadMultipleFiles.setInputFiles([
  53 |             './data/loginData.csv',
  54 |             './data/loginData.json'
  55 |         ]);
  56 | 
  57 |         let tableRow = await staticTable.locator('tbody tr: nth-child(2)');
> 58 |         console.log(`Static Table Row content: ${tableRow.innerText()}`);
     |                                                           ^ Error: locator.innerText: Unexpected token " " while parsing css selector "tbody tr: nth-child(2)". Did you mean to CSS.escape it?
  59 | 
  60 |     });
