...
Call log:
  - waiting for getByRole('listitem', { name: 'BANK' })
...
```ts
  ...
  76  | 
  77  |         let optionBank = await page.getByRole('listitem', {name: "BANK"}); 
  78  |         let optionSales = await page.getByRole('listitem', {name: "SALES"}); 
...
> 83  |         await optionBank.hover();
      |                          ^ Error: locator.hover: Target page, context or browser has been closed
  84  |         await page.mouse.down();
  85  |         await targetDebit.hover();
  86  |         await page.mouse.up();
  87  | 
  88  |         await optionSales.hover();
  89  |         await page.mouse.down();
  90  |         await targetCredit.hover();
  91  |         await page.mouse.up();
  92  | 
  93  |         
  94  |     });
  95  |     test.afterEach('Close Page', async() => {
  96  |         await page.close();
  97  |     });
  98  |     test.afterAll('Close Browser', async() => {
