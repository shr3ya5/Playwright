...
```ts
  ...
  57 |         });
  58 |         await test.step('Print Flight Details for First AI Express Flight', async () => {
> 59 |             console.log(`Flight Details: ${await departTime.innerText()}, ${await arrivalTime.innerText()}, 
     |                                                             ^ Error: locator.innerText: Target page, context or browser has been closed
  60 |                                     +       ${await totalJourneyTime.innerText()}, ${await journeyType.innerText()}`);
  61 |         });
  62 |     });
  63 | });
```