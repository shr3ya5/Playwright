...
Call log:
  - waiting for locator('table[name="BookTable"] tbody tr:nth-child(1) td:nth-child(2)')
...
> 32 |         let tableRow = await page.locator('table[name="BookTable"] tbody tr:nth-child(1) td:nth-child(2)').innerText();
     |                                                                                                            ^ Error: locator.innerText: Target page, context or browser has been closed
  33 | 
  ...
  51 |         console.log(`Static Table Row content: ${tableRow}`);
  52 | 
  53 |         await uploadFile.setInputFiles('./data/Push_Playwright_Code_to_GitHub_Step_by_Step.txt');
  54 |         await uploadMultipleFiles.setInputFiles([
  55 |             './data/loginData.csv',
  56 |             './data/loginData.json'
  57 |         ]);
  58 | 
  59 |     });
  60 |     test.afterAll('Cleanup', async() => {
  61 |         await page.close();
