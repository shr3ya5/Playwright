...
Call log:
  - waiting for locator('#dropdown-menu').locator('ul#dropdown-menu li').filter({ hasText: 'Apple' })
...
```ts
  ...
  36 | 
  37 |         let AppleOption = await selectFruitsDD.locator('ul#dropdown-menu li').filter({hasText: "Apple"});
> 38 |         await AppleOption.click();
     |                           ^ Error: locator.click: Target page, context or browser has been closed
  39 | 
  40 |         let BananaOption = await selectFruitsDD.locator('ul#dropdown-menu li').filter({hasText: "Banana"});
