...
```ts
  ...
  36 | 
  37 |         let AppleOption = await page.locator('#dropdown-menu label').filter({hasText: 'Apple'});
  38 |         await AppleOption.click();
  39 | 
  40 |         let BananaOption = await page.locator('#dropdown-menu label').filter({hasText: 'Banana'});
