...
Call log:
  - waiting for locator('#dropdown-menu').getByLabel('Apple')
...
```ts
  ...
  34 | 
  35 |         let AppleOption = selectFruitsDD.getByLabel('Apple');
> 36 |         await AppleOption.check();
     |                           ^ Error: locator.check: Clicking the checkbox did not change its state
  37 |         await expect(AppleOption).toBeChecked();
  38 | 
  39 |         let BananaOption = selectFruitsDD.getByLabel('Banana');
